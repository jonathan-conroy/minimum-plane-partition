...
int main()
{
    ...
    for (int i = 0; i < numSegments; i++) {
        ...
        Point_2 s = segments[i].source();
        Point_2 t = segments[i].target();
        // std::cout << "Segment: (" << std::to_string(s.x()) << ", "
        //                               << std::to_string(s.y()) << ") and ("
        //                               << std::to_string(t.x()) << ", "
        //                               << std::to_string(t.y()) << ")" << std::endl;
        std::cout << "Segment: " << segments[i] << std::endl;

        for (int j = 0; j < i; j++) {
            Segment_2 curr_pair[] = {segments[i], segments[j]};
            // std::cout << "Segment2: (" << std::to_string(segments[j].point(0)[0]) << ", "
            //                           << std::to_string(segments[j].point(0)[1]) << ") and ("
            //                           << std::to_string(segments[j].point(1)[0]) << ", "
            //                           << std::to_string(segments[j].point(1)[1]) << ")" << std::endl;
